...
| | 11 | 🧬 **分子生成** | — | LSTM 自回归生成新颖候选分子（内置数据集为 EGFR 抑制剂） |
| ⚡ 整合 | 12 | ⚙️ **自动化流程** | — | 预测→筛选→药效团→相似性一键串联 |
| 🎓 教学 | 13 | 🎓 **教学实验室** | — | 下载真实数据 → 清洗（看清流失）→ 现场训练 RF/GNN → 预测新分子 |
| 📊 总结 | 14 | 📊 **模型与系统** | 📈性能 · 🏗️架构 · 📚关于 | 模型评估 + 双引擎架构 + 技术栈 + 项目背景 |
...
| 🎓 教学层 | 教学实验室 | 任意靶点：下载 → 清洗 → 现场训练 → 预测（四步闭环） |
...
```
.
├── app.py                      # 主入口（st.navigation 14 页，5 组合并标签）
...
| 12 | 🎓 **教学实验室** → 换一个靶点，从下载到预测亲手走完四步 | 看清「答案是怎么来的」：数据、清洗与训练全流程 |
| 13 | 📊 **模型与系统** → 查看性能指标与系统架构 | 总结反思，理解全局 |
...
平台**不限定靶点**：📦 数据获取、🎓 教学实验室 等页面都接受任意靶点名（HER2、BTK、CDK2…），可检索 ChEMBL 中该靶点的真实活性数据并走完清洗 / 建模流程。
